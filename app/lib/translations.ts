export type Lang = "en" | "es";

export const translations = {
  en: {
    nav: {
      mission: "Mission",
      founders: "Founders",
      pasalapro: "PasalaPro",
      sponsors: "Sponsors",
      founderEdition: "Founder Edition",
    },
    hero: {
      tag: "Powered by PasalaPro",
      subheadline: "One Club. One Community. One Movement.",
      body: "A new football vision begins. Founder Edition — Numbered. Limited. Official.",
      cta1: "Reserve Founder Edition",
      cta2: "View Founder Registry",
      stat1Label: "Founder Jerseys",
      stat2Label: "El Salvador",
      stat3Label: "Pasala",
    },
    mision: {
      tag: "Our Mission",
      headlineA: "Created to",
      headlineB: "open doors",
      body: "Harpia FC was born with a clear purpose: to create opportunities for undiscovered football talents. From the community to the pitch.",
      quote: "A new era for Salvadoran football.",
      quoteAttrib: "— Harpia FC",
      pillars: [
        {
          title: "Real Visibility",
          text: "We bring talents out of the shadows. Connecting undiscovered players with scouts, sponsors, and real opportunities.",
        },
        {
          title: "Complete Ecosystem",
          text: "Through PasalaPro, teams, players, fans and brands come together in one place. It's not just a club — it's a platform.",
        },
        {
          title: "From the Community",
          text: "Harpia FC doesn't come from the top. It comes from the pitches, the neighborhoods, and those who love Salvadoran football.",
        },
        {
          title: "Identity & Legacy",
          text: "Every jersey, every match, every step builds something bigger. Harpia FC is the start of a legacy El Salvador deserves.",
        },
      ],
    },
    fundadores: {
      sectionLabel: "OFFICIAL FOUNDER COLLECTION",
      title: "Founder Edition Reservation",
      subtitle: "$250 USD · Limited to 1,000 Founder Editions",
      mainCopy: [
        "Reserve your place in Harpia FC history.",
        "The official Founder Edition is limited, numbered, and created for those who believe from day one.",
        "Each reservation secures your permanent place in the Official Harpia FC Founder Registry, along with a collectible Founder Edition accompanied by a laser-engraved metal Certificate of Authenticity.",
        "The number on your certificate matches the number on your jersey — making it a unique, authenticated piece in the history of the club.",
        "Once all 1,000 numbers have been reserved, the Founder Collection will be permanently closed. There will never be a second Founder Edition.",
      ],
      features: [
        "Reserved Founder Number",
        "Entry into the Official Harpia FC Founder Registry",
        "Founder Status within the movement",
        "Collectible Founder Jersey",
        "Laser-Engraved Metal Certificate of Authenticity",
        "Matching Jersey and Certificate Number",
        "Premium Collector Presentation Box",
        "Permanent place in the first official Harpia FC collection",
      ],
      scarcity: {
        title: "A reservation that will only happen once.",
        reservations: [
          { number: "#0001", owner: "PasalaPro", desc: "Official technology sponsor and co-founder of the movement.", color: "#0AAFAA" },
          { number: "#0002–0025", owner: "Team Reserve", desc: "Reserved for the founders and core team behind Harpia FC.", color: "#C9A84C" },
          { number: "#0026–0030", owner: "Community Reserve", desc: "Reserved for key early community members.", color: "#C0C0C0" },
          { number: "#0031+", owner: "Public", desc: "Public Founder Editions begin at #0031. Only 970 reservations available.", color: "#4ADE80", pulse: true },
        ],
        statements: [
          "Once all 1,000 numbers have been reserved, the Founder Collection will be permanently closed.",
          "There will never be a second Founder Edition.",
        ],
      },
      certificate: {
        title: "Collectible Certificate of Authenticity",
        body: [
          "Each Founder Edition includes a premium metal Certificate of Authenticity engraved with laser precision.",
          "Every certificate carries the exact same unique number as its corresponding Founder Jersey.",
          "The Founder Jersey, Certificate, and Premium Collector Presentation Box together form a single authenticated collectible piece.",
        ],
      },
      registry: {
        title: "Official Founder Registry",
        body: [
          "Every Founder Edition will be permanently recorded within the Official Harpia FC Founder Registry.",
          "Each number represents a unique piece of club history.",
          "Owners may choose to appear publicly within the registry or remain private.",
        ],
        features: [
          "Official ownership registry",
          "Unique and irreplaceable number",
          "Verifiable ownership history",
          "Permanent place in Harpia FC history",
        ],
      },
      marketplace: {
        title: "Built for Collectors",
        body: [
          "Harpia FC plans to maintain an official ownership and transfer registry for Founder Editions.",
          "This future system is intended to preserve authenticity, ownership history, and provenance for every numbered piece in the collection.",
        ],
      },
      internalCta: {
        headline: "Secure your Founder Number. Help build Harpia FC.",
        body: "By reserving your Founder Edition today, you become one of the first 1,000 supporters helping fund the creation of Harpia FC. Your number will be permanently recorded in the Official Founder Registry — part of this club's history forever.",
        button: "RESERVE MY FOUNDER EDITION",
        secondaryButton: "VIEW FOUNDER REGISTRY",
        price: "$250 USD",
        publicNote: "Public Founder Editions begin at No. 0031",
        disclaimer: "Only 1,000 Founder Editions will ever exist. No reprints. No exceptions.",
      },
    },
    pasalapro: {
      tag: "Official Sponsor",
      body: "PasalaPro is the technology platform powering Harpia FC's digital ecosystem — connecting teams, players, sponsors, and fans in one place.",
      cardTitle: "The platform",
      cardTitleAccent: "behind the movement",
      cardBody1: "PasalaPro helps organize football communities with tools for teams, leagues, communication, payments, sponsors, and fan engagement.",
      cardBody2: "For Harpia FC, PasalaPro becomes the engine that connects the club, its supporters, and its future football ecosystem from day one.",
      stats: [
        { value: "1", label: "Unified platform" },
        { value: "4", label: "User types" },
        { value: "∞", label: "Possible connections" },
        { value: "+", label: "Built to scale" },
      ],
      features: [
        { title: "Players", desc: "Digital profile, stats and visibility to scouts and clubs." },
        { title: "Teams", desc: "Roster management, calendars, results and centralized communication." },
        { title: "Sponsors", desc: "Direct connection with clubs and players with real impact metrics." },
        { title: "Community", desc: "Fans, creators and followers connected to the club ecosystem." },
      ],
    },
    patrocinadores: {
      tag: "Sponsorship opportunities",
      headlineA: "Support",
      headlineB: "Harpia FC",
      headlineC: "",
      body: "Sponsoring Harpia FC at this foundational stage is more than brand visibility — it's being part of a historic movement for Salvadoran football. The brands that come first will always be remembered.",
      tiers: [
        {
          name: "Gold Sponsor",
          perks: [
            "Logo on the official jersey",
            "Presence across all digital channels",
            "Exclusive access to player data",
            "Club events and activations",
            "Mentions in associated media",
          ],
        },
        {
          name: "Silver Sponsor",
          perks: [
            "Logo on digital materials",
            "Presence on club social media",
            "Access to club events",
            "Brand association with the movement",
          ],
        },
        {
          name: "Community Ally",
          perks: [
            "Official ally mention",
            "Presence on digital channels",
            "Connection with the Harpia community",
          ],
        },
      ],
      note: "Packages are customizable. Contact the team for details.",
      cta: "Sponsor Harpia FC",
    },
    cta: {
      headlineA: "Become part of the",
      headlineB: "birth",
      headlineC: "of Harpia FC",
      body: "The future of Salvadoran football starts from zero. The first 1,000 are the ones who write history.",
      quote: "From the community to the pitch.",
      counterTotalLabel: "Total jerseys",
      counterStartLabel: "Starts here",
      counterImpactLabel: "Impact",
      btnReserve: "Reserve my Founder Edition",
      btnContact: "View Founder Registry",
    },
    footer: {
      tagline: "One Club. One Community. One Movement. The future of Salvadoran football starts from zero.",
      poweredBy: "Powered by PasalaPro",
      navTitle: "Navigation",
      navLinks: [
        { label: "Home", href: "#" },
        { label: "Mission", href: "#mision" },
        { label: "Founder Edition", href: "#fundadores" },
        { label: "PasalaPro", href: "#pasalapro" },
        { label: "Sponsors", href: "#patrocinadores" },
      ],
      contactTitle: "Contact",
      copyright: "All rights reserved.",
      techBy: "Technology by",
    },
    checkout: {
      pageTitle: "Founder Edition Reservation",
      sectionLabel: "SECURE YOUR FOUNDER NUMBER",
      price: "$250 USD",
      priceNote: "A processing fee will be added at checkout.",
      installmentsNote: "Pay a deposit today, complete the balance in installments.",
      fields: {
        firstName: "First Name",
        lastName: "Last Name",
        email: "Email Address",
        phone: "Phone (optional)",
        country: "Country",
        jerseySize: "Jersey Size",
        jerseyName: "Name on Jersey (optional)",
        jerseyNumber: "Number on Jersey (optional)",
        certificateName: "Name on Certificate",
        certificateNameHint: "Exactly as you want it on your metal certificate. Max 60 characters.",
        displayName: "Public Registry Name",
        displayNameHint: "Name shown in the Official Founder Registry. Max 50 characters.",
        isPublic: "Appear in the public Founder Registry",
        paymentPlan: "Payment Plan",
        planFull: "Pay in full — $250 USD",
        planInstallments: "Pay in installments",
        deposit: "Deposit Amount (USD)",
        depositHint: "Minimum $50. You will complete the balance in installments.",
        termsLabel: "I have read and accept the",
        termsLink: "Terms & Conditions",
      },
      submit: "PROCEED TO CHECKOUT",
      submitting: "Creating your order…",
      errors: {
        generic: "Something went wrong. Please try again.",
        invalidUrl: "Invalid redirect URL. Please try again.",
      },
    },
    success: {
      pageTitle: "Reservation Confirmed",
      loading: "Confirming your reservation…",
      polling: "Your payment is being processed. This may take a few seconds…",
      paid: "Your Founder Edition is confirmed!",
      partial: "Deposit received. Your reservation is active.",
      pending: "Payment processing. Check back in a moment.",
      founderNumber: "Your Founder Number",
      amountPaid: "Amount Paid",
      balance: "Remaining Balance",
      nextSteps: "What happens next?",
      nextStepsList: [
        "You will receive a confirmation email shortly.",
        "Your number is permanently reserved in the Official Founder Registry.",
        "Your Founder Edition will be delivered once production is complete.",
      ],
      viewRegistry: "View Founder Registry",
      shippingNote: "Your shipping details were collected securely at checkout.",
      backHome: "Back to Home",
    },
    cancel: {
      pageTitle: "Checkout Cancelled",
      heading: "Your reservation was not completed.",
      body: "No charges were made. Your information is still saved — you can resume checkout at any time.",
      resumeCheckout: "Resume Checkout",
      startOver: "Start a New Reservation",
      backHome: "Back to Home",
      resuming: "Preparing your checkout…",
    },
  },

  es: {
    nav: {
      mission: "Misión",
      founders: "Fundadores",
      pasalapro: "PasalaPro",
      sponsors: "Patrocinadores",
      founderEdition: "Edición Fundadores",
    },
    hero: {
      tag: "Impulsado por PasalaPro",
      subheadline: "Un club. Una comunidad. Un movimiento.",
      body: "Una nueva visión del fútbol comienza. Edición Fundadores — Numerada. Limitada. Oficial.",
      cta1: "Reservar Edición Fundadores",
      cta2: "Ver Registro Fundadores",
      stat1Label: "Jerseys Fundadores",
      stat2Label: "El Salvador",
      stat3Label: "Pasala",
    },
    mision: {
      tag: "Nuestra Misión",
      headlineA: "Creado para",
      headlineB: "abrir puertas",
      body: "Harpia FC nació con un propósito claro: crear oportunidades para talentos futbolísticos no descubiertos. De la comunidad para la cancha.",
      quote: "Una nueva era para el fútbol salvadoreño.",
      quoteAttrib: "— Harpia FC",
      pillars: [
        {
          title: "Visibilidad real",
          text: "Sacamos a los talentos de la sombra. Conectamos jugadores no descubiertos con scouts, patrocinadores y oportunidades reales.",
        },
        {
          title: "Ecosistema completo",
          text: "A través de PasalaPro, equipos, jugadores, fans y marcas se encuentran en un solo lugar. No es solo un club — es una plataforma.",
        },
        {
          title: "De la comunidad",
          text: "Harpia FC no viene de arriba. Viene de las canchas, de los barrios, de quienes aman el fútbol salvadoreño y creen en su futuro.",
        },
        {
          title: "Identidad y legado",
          text: "Cada jersey, cada partido, cada paso construye algo más grande. Harpia FC es el inicio de un legado que El Salvador merece.",
        },
      ],
    },
    fundadores: {
      sectionLabel: "COLECCIÓN OFICIAL DE FUNDADORES",
      title: "Reserva de Edición Fundadores",
      subtitle: "$250 USD · Limitada a 1,000 Ediciones Fundadores",
      mainCopy: [
        "Reserva tu lugar en la historia de Harpia FC.",
        "La Edición Fundadores oficial es limitada, numerada, y creada para quienes creen desde el primer día.",
        "Cada reserva asegura tu lugar permanente en el Registro Oficial de Fundadores de Harpia FC, junto con una Edición Fundadores coleccionable acompañada de un Certificado de Autenticidad metálico grabado con láser.",
        "El número en tu certificado coincide con el número en tu jersey — convirtiéndola en una pieza única y autenticada en la historia del club.",
        "Una vez que los 1,000 números sean reservados, la Colección Fundadores quedará cerrada permanentemente. No habrá una segunda Edición Fundadores.",
      ],
      features: [
        "Número de Fundador Reservado",
        "Entrada al Registro Oficial de Fundadores de Harpia FC",
        "Estatus de Fundador dentro del movimiento",
        "Jersey Fundador Coleccionable",
        "Certificado de Autenticidad Metálico Grabado con Láser",
        "Número Coincidente en Jersey y Certificado",
        "Caja de Presentación Premium para Coleccionistas",
        "Lugar permanente en la primera colección oficial de Harpia FC",
      ],
      scarcity: {
        title: "Una reserva que solo ocurrirá una vez.",
        reservations: [
          { number: "#0001", owner: "PasalaPro", desc: "Patrocinador tecnológico oficial y co-fundador del movimiento.", color: "#0AAFAA" },
          { number: "#0002–0025", owner: "Reserva Equipo", desc: "Reservadas para los fundadores y el equipo central detrás de Harpia FC.", color: "#C9A84C" },
          { number: "#0026–0030", owner: "Reserva Comunidad", desc: "Reservadas para miembros clave de la comunidad inicial.", color: "#C0C0C0" },
          { number: "#0031+", owner: "Público", desc: "Las Ediciones Fundadores públicas comienzan en el #0031. Solo quedan 970 reservas disponibles.", color: "#4ADE80", pulse: true },
        ],
        statements: [
          "Una vez reservados los 1,000 números, la Colección Fundadores quedará cerrada permanentemente.",
          "Nunca habrá una segunda Edición Fundadores.",
        ],
      },
      certificate: {
        title: "Certificado Coleccionable de Autenticidad",
        body: [
          "Cada Edición Fundadores incluye un Certificado de Autenticidad metálico premium grabado con precisión láser.",
          "Cada certificado lleva exactamente el mismo número único que su Jersey Fundador correspondiente.",
          "El Jersey Fundador, el Certificado y la Caja de Presentación Premium juntos forman una sola pieza coleccionable autenticada.",
        ],
      },
      registry: {
        title: "Registro Oficial de Fundadores",
        body: [
          "Cada Edición Fundadores quedará registrada permanentemente en el Registro Oficial de Fundadores de Harpia FC.",
          "Cada número representa una pieza única de la historia del club.",
          "Los propietarios pueden elegir aparecer públicamente dentro del registro o permanecer en privado.",
        ],
        features: [
          "Registro oficial de propiedad",
          "Número único e irremplazable",
          "Historial de propiedad verificable",
          "Lugar permanente en la historia de Harpia FC",
        ],
      },
      marketplace: {
        title: "Construido para Coleccionistas",
        body: [
          "Harpia FC planea mantener un registro oficial de propiedad y transferencias para las Ediciones Fundadores.",
          "Este futuro sistema está destinado a preservar la autenticidad, el historial de propiedad y la procedencia de cada pieza numerada en la colección.",
        ],
      },
      internalCta: {
        headline: "Asegura tu Número de Fundador. Ayuda a construir Harpia FC.",
        body: "Al reservar tu Edición Fundadores hoy, te conviertes en uno de los primeros 1,000 seguidores que ayudan a financiar la creación de Harpia FC. Tu número quedará registrado permanentemente en el Registro Oficial de Fundadores — parte de la historia de este club para siempre.",
        button: "RESERVAR MI EDICIÓN FUNDADORES",
        secondaryButton: "VER REGISTRO FUNDADORES",
        price: "$250 USD",
        publicNote: "Las Ediciones Fundadores públicas comienzan en el No. 0031",
        disclaimer: "Solo existirán 1,000 Ediciones Fundadores. Sin reimpresión. Sin excepciones.",
      },
    },
    pasalapro: {
      tag: "Patrocinador Oficial",
      body: "PasalaPro es la plataforma tecnológica que impulsa el ecosistema digital de Harpia FC — conectando equipos, jugadores, patrocinadores y fans en un solo lugar.",
      cardTitle: "La plataforma",
      cardTitleAccent: "detrás del movimiento",
      cardBody1: "PasalaPro ayuda a organizar comunidades de fútbol con herramientas para equipos, ligas, comunicación, pagos, patrocinadores y participación de fans.",
      cardBody2: "Para Harpia FC, PasalaPro se convierte en el motor que conecta el club, sus seguidores y su futuro ecosistema futbolístico desde el primer día.",
      stats: [
        { value: "1", label: "Plataforma unificada" },
        { value: "4", label: "Tipos de usuarios" },
        { value: "∞", label: "Conexiones posibles" },
        { value: "+", label: "Construido para escalar" },
      ],
      features: [
        { title: "Jugadores", desc: "Perfil digital, estadísticas y visibilidad ante reclutadores y clubes." },
        { title: "Equipos", desc: "Gestión de plantel, calendarios, resultados y comunicación centralizada." },
        { title: "Patrocinadores", desc: "Conexión directa con clubes y jugadores con métricas de impacto real." },
        { title: "Comunidad", desc: "Fans, creadores y seguidores conectados al ecosistema del club." },
      ],
    },
    patrocinadores: {
      tag: "Oportunidades de patrocinio",
      headlineA: "Apoya",
      headlineB: "Harpia FC",
      headlineC: "",
      body: "Patrocinar Harpia FC en esta etapa fundacional es más que visibilidad de marca — es ser parte de un movimiento histórico para el fútbol salvadoreño. Las marcas que entren primero, serán recordadas siempre.",
      tiers: [
        {
          name: "Patrocinador Oro",
          perks: [
            "Logo en el jersey oficial",
            "Presencia en todos los canales digitales",
            "Acceso exclusivo a datos de jugadores",
            "Eventos y activaciones del club",
            "Mención en medios asociados",
          ],
        },
        {
          name: "Patrocinador Plata",
          perks: [
            "Logo en materiales digitales",
            "Presencia en redes del club",
            "Acceso a eventos del club",
            "Asociación de marca con el movimiento",
          ],
        },
        {
          name: "Aliado Comunitario",
          perks: [
            "Mención como aliado oficial",
            "Presencia en canales digitales",
            "Conexión con la comunidad Harpia",
          ],
        },
      ],
      note: "Los paquetes son personalizables. Contacta al equipo para más detalles.",
      cta: "Patrocinar Harpia FC",
    },
    cta: {
      headlineA: "Sé parte del",
      headlineB: "nacimiento",
      headlineC: "de Harpia FC",
      body: "El futuro del fútbol salvadoreño empieza desde cero. Los primeros 1,000 son los que escriben la historia.",
      quote: "De la comunidad para la cancha.",
      counterTotalLabel: "Jerseys totales",
      counterStartLabel: "Desde aquí",
      counterImpactLabel: "Impacto",
      btnReserve: "Reservar mi Edición Fundadores",
      btnContact: "Ver Registro Fundadores",
    },
    footer: {
      tagline: "Un club. Una comunidad. Un movimiento. El futuro del fútbol salvadoreño empieza desde cero.",
      poweredBy: "Impulsado por PasalaPro",
      navTitle: "Navegación",
      navLinks: [
        { label: "Inicio", href: "#" },
        { label: "Misión", href: "#mision" },
        { label: "Edición Fundadores", href: "#fundadores" },
        { label: "PasalaPro", href: "#pasalapro" },
        { label: "Patrocinadores", href: "#patrocinadores" },
      ],
      contactTitle: "Contacto",
      copyright: "Todos los derechos reservados.",
      techBy: "Tecnología de",
    },
    checkout: {
      pageTitle: "Reserva de Edición Fundadores",
      sectionLabel: "ASEGURA TU NÚMERO DE FUNDADOR",
      price: "$250 USD",
      priceNote: "Se añadirá una tarifa de procesamiento al momento del pago.",
      installmentsNote: "Paga un depósito hoy y completa el saldo en cuotas.",
      fields: {
        firstName: "Nombre",
        lastName: "Apellido",
        email: "Correo electrónico",
        phone: "Teléfono (opcional)",
        country: "País",
        jerseySize: "Talla de Jersey",
        jerseyName: "Nombre en el jersey (opcional)",
        jerseyNumber: "Número en el jersey (opcional)",
        certificateName: "Nombre en el certificado",
        certificateNameHint: "Tal como quieres que aparezca en tu certificado metálico. Máx. 60 caracteres.",
        displayName: "Nombre en el Registro Público",
        displayNameHint: "Nombre que aparecerá en el Registro Oficial de Fundadores. Máx. 50 caracteres.",
        isPublic: "Aparecer en el Registro Público de Fundadores",
        paymentPlan: "Plan de pago",
        planFull: "Pago completo — $250 USD",
        planInstallments: "Pago en cuotas",
        deposit: "Monto del depósito (USD)",
        depositHint: "Mínimo $50. Completarás el saldo restante en cuotas.",
        termsLabel: "He leído y acepto los",
        termsLink: "Términos y Condiciones",
      },
      submit: "PROCEDER AL PAGO",
      submitting: "Creando tu orden…",
      errors: {
        generic: "Algo salió mal. Por favor intenta de nuevo.",
        invalidUrl: "URL de redirección inválida. Por favor intenta de nuevo.",
      },
    },
    success: {
      pageTitle: "Reserva Confirmada",
      loading: "Confirmando tu reserva…",
      polling: "Tu pago está siendo procesado. Esto puede tardar unos segundos…",
      paid: "¡Tu Edición Fundadores está confirmada!",
      partial: "Depósito recibido. Tu reserva está activa.",
      pending: "Procesando pago. Vuelve en un momento.",
      founderNumber: "Tu Número de Fundador",
      amountPaid: "Monto Pagado",
      balance: "Saldo Restante",
      nextSteps: "¿Qué sigue?",
      nextStepsList: [
        "Recibirás un correo de confirmación en breve.",
        "Tu número está reservado permanentemente en el Registro Oficial de Fundadores.",
        "Tu Edición Fundadores será entregada cuando la producción esté lista.",
      ],
      viewRegistry: "Ver Registro Fundadores",
      shippingNote: "Tu dirección de envío fue recopilada de forma segura en el pago.",
      backHome: "Volver al inicio",
    },
    cancel: {
      pageTitle: "Pago Cancelado",
      heading: "Tu reserva no fue completada.",
      body: "No se realizó ningún cargo. Tu información sigue guardada — puedes retomar el pago cuando quieras.",
      resumeCheckout: "Retomar pago",
      startOver: "Comenzar una nueva reserva",
      backHome: "Volver al inicio",
      resuming: "Preparando tu pago…",
    },
  },
} as const;

export type Translations = typeof translations;
